...
export class MyAppointmentsComponent implements OnInit {
  ...
  showConfirm(message: string, callback: () => void, type: 'info' | 'danger' | 'warning' | 'success' = 'warning', title = 'Confirmar') {
    this.modalConfig.set({
      title, message, type,
      icon: type === 'danger' ? 'ph ph-trash' : 'ph ph-warning'
    });
  ...
  selectDate(day: {date: string; day: number; disabled: boolean; past: boolean}): void {
    ...
    this.selectedSlot.set(null); // Limpiar slot previo si cambia fecha
    this.loadSlots();
  }
  ...
  contactWhatsApp(cita: CitaPatient): void {
    ...
    const message = encodeURIComponent(`[DENTAL-PORTAL] Hola, necesito información sobre mi cita folio ${cita.folio} (${cita.servicioNombre}).`);
  ...
  requestCancellationWA(cita: CitaPatient): void {
    ...
    const phone = cita.sucursalTelefono.replace(/\D/g, '');
    const message = encodeURIComponent(`[ACCION:CANCELAR] Hola, deseo CANCELAR mi cita confirmada con folio ${cita.folio} programada para el día ${this.formatDate(cita.fechaHora)} a las ${new Date(cita.fechaHora).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}hs.`);
    window.open(`https://wa.me/52${phone}?text=${message}`, '_blank');
  }
  ...
  requestRescheduleWA(cita: CitaPatient): void {
    if (!cita.sucursalTelefono) return;
    const phone = cita.sucursalTelefono.replace(/\D/g, '');
    const message = encodeURIComponent(`[ACCION:REAGENDAR] Hola, deseo REAGENDAR mi cita confirmada con folio ${cita.folio} programada para el día ${this.formatDate(cita.fechaHora)} a las ${new Date(cita.fechaHora).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}hs. ¿Qué horarios tienen disponibles?`);
    window.open(`https://wa.me/52${phone}?text=${message}`, '_blank');
  }
